...
router.post("/", passport.authenticate("jwt", { session: false }), async (req, res) => {
    ...
    const post = await PostService.create(body.title, body.content, body.published, user._id.toString());
    if (post) {
...
router.get("/:id/protected", passport.authenticate("jwt", { session: false }), async (req, res) => {
    const user = req.user;
...
router.patch("/:id", passport.authenticate("jwt", { session: false }), async (req, res) => {
    ...
    const updated = await PostService.updateById(post?.id, body.title, body.content, body.published);
    if (updated) {
        return res.status(200).json(post);
    }
    else {
...
router.post("/:id/likes", passport.authenticate("jwt", { session: false }), async (req, res) => {
    const body = req.body;
    const user = req.user;
